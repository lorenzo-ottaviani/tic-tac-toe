...
def jouer_ia(plateau):
    cases_vides = [i for i, case in enumerate(plateau) if case == ' ']
    return random.choice(cases_vides)

def jeu_morpion_ia():
    plateau = [' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ']
    joueur_actuel = 'X'  # X commence

    for tour in range(9):
        afficher_plateau(plateau)
        
        if joueur_actuel == 'X':
            choix = int(input("Joueur X, choisissez une case (1-9): ")) - 1
            if plateau[choix] == ' ':
                plateau[choix] = 'X'
            else:
            ...
            choix = jouer_ia(plateau)
            plateau[choix] = 'O'
            print(f"L'IA a joué en case {choix + 1}")
